...
<div class="modal-overlay" id="orderModal">
    <div class="modal">
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-success" id="btnApprove" onclick="askPassword('approve')">승인</button>
        </div>
    </div>
</div>
...
<script>
    ...
    function renderTable() {
        ...
        tbody.innerHTML = list.map(function(o) {
            ...
            return '<tr>'
                ...
                + '<td class="num">' + fmtDate(o.createDate) + '</td>'
    ...
    function openOrderModal(id) {
        ...
        document.getElementById('mCreateDate').textContent       = fmtDate(order.createDate);
        ...
        if (isPending) {
            ...
            document.getElementById('approverDetail').innerHTML =
                '<strong>' + label + '</strong>'
                + '<br><span style="color:var(--text-muted); font-size:12px;">처리 시각: ' + fmtDateTime(order.updateDate) + '</span>';
        }
    ...
    async function submitPassword() {
        ...
        try {
            if (pendingAction.action === 'approve')      { await approveOrder(pendingAction.orderId, password); alert('승인 완료!'); }
            else if (pendingAction.action === 'reject')  { await rejectOrder(pendingAction.orderId, password);  alert('반려 처리됨'); }
            else if (pendingAction.action === 'update')  { await updateOrder(pendingAction.orderId, password);  alert('수정 저장됨!'); }
            closePwModal();
            closeOrderModal();
            await Promise.all([loadOrders('pending'), loadOrders('completed'), loadOrders('rejected')]);
            updateCounts();
