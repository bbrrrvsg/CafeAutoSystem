...
public class AiPredictController {
    ...
    @PostMapping("/draft")
    public ResponseEntity<PurchaseOrder> createDraft(@RequestBody PurchaseOrder order) {
        PurchaseOrder saved = aiPredictService.createAiDraftOrder(order);
        return ResponseEntity.ok(saved);
    }

    // [Read] 생성된 모든 발주서 리스트 확인 API
    @GetMapping("/orders")
    public ResponseEntity<List<PurchaseOrder>> getOrderList() {
    ...
    public ResponseEntity<List<HistoricalStockLog>> getHistoricalLogs(@PathVariable("ingredientId") Integer ingredientId) {
